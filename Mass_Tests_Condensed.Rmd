...
The tables below show the movement durations (\@ref{rab:utilmovedurstab}), reaction time (\@ref{rab:utilreacttimestab}).
...
The next table (\@ref(tab:utilcoefftabs)) shows the fitted values for alpha given the specific utility model and the parameters fit previously. These are only for experiment 2a. 
...
This table (\@ref(tab:utildurs2a)) shows the duration from experiment 2a, along with the optimized predictions for each of the utility/effort models. The SSE for all these models is discussed later (section \@ref{SSE2a}).
...
```{r alphavalues, echo = FALSE, warning = FALSE, message = FALSE, tab.cap = '$\alpha$ fits and SSE for the different experiements'}
tab = cbind(c('Utility 2a Net','Utility 2b Gross','Utility Gross 2a and 2b','Utility 2b Gross','Utility 2c Gross'),
            ...
            c(formatC(sum((mvttimes_pref-util_dur_net_rx)^2),format = 'e', digits = 3),
              formatC(sum((mvttimes_pref-util_dur_gross_rx)^2),format = 'e', digits = 3),
              formatC(sum((mvttimes_pref-util_dur_pref_alpha_comb_gross[1:4])^2)+sum((mvttimes_smallt -util_dur_pref_alpha_comb_gross[5:8])^2),
                      format = 'e', digits = 3),
              formatC(sum((mvttimes_smallt-util_dur_smallt)^2),format = 'e', digits = 3),
              formatC(sum((mvttimes_pilot-util_dur_pilot)^2),format = 'e', digits = 3)))
colnames(tab) = c('Experiments fit','$\alpha$','SSE')
kable(tab, caption = '$\alpha$ fits and SSE for the different experiements.')
```
...
The probability of success for utility in experiement 2b using the optimized alpha value are shown below. 2a can be found above in \@ref{tab:optprobstab2a}

```{r optprobstabcomb, echo = FALSE, warning = FALSE, message = FALSE}
...
This table (\@ref(tab:utildurscombalpha)) shows the duration from experiment 2a/2b. The first table is one alpha fitted to 2a and 2b. Second table, one alpha for 2b. Third table one alpha for 2c.

```{r utildurscombalpha, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r ModelSSEnorm12b, echo = FALSE, warning = FALSE, tab.cap = 'Exp 2b - Sum squared errors for NORMALIZED movement duration and peak velocity.'}
...
```{r ModelSSE12c, echo = FALSE, warning = FALSE, tab.cap = 'Exp 2c - Sum squared errors for movement duration and peak velocity.'}
...
```{r ModelSSEnorm12c, echo = FALSE, warning = FALSE, tab.cap = 'Exp 2c - Sum squared errors for NORMALIZED movement duration and peak velocity.'}
