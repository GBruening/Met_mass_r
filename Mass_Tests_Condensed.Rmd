...
<!-- 8/6 DO -->
<!-- Get AI Files -->
<!-- do a logistic for experiment 2a given prob success stuff -->
  <!-- Add these as dots the probability of success -->
<!-- Utility model the probs calcs with glm? -->

<!-- 8/8 done -->
<!-- Do test on probability of reward on mass -->
<!-- AIC SSE for torque squared not fit to 0 -->
  <!-- Move it up to the effort model -->
<!-- Make torque rate plots similar to met plots with error bars -->
<!-- Fix interaction effect string, increase and decrease is switched -->
<!-- All net stuff should just take gross a and minus a_resting -->
<!-- relabel 2a as 2a and so on -->
<!-- Need to add reaction times to utility modeling section -->
  <!-- Split talbes up because apaprently a spread sheet is easier to understand -->
  
<!-- 8/3 done -->
<!-- add SSE for probability prediction on 2b -->
<!-- Fix metabolic results alignment -->
<!-- Find maximum utility by just changing probability, given alpha from 2a. -->
  <!-- Add to SSE tables -->

<!-- 7/27 Done -->
<!-- Make the plots for the preferred experiement without the violins, add standard error bars for each experiment -->
<!-- Make optimal durations smaller -->
<!-- Add line for resting rate in gross then remove net metabolics -->
<!-- Add SSE for the effort model fits with the AIC /BIC -->

<!-- 7/23 Things to do -->
<!-- Remove net metabolic power -->
<!--   Think about it -->
<!-- Remove data points from cost plots -->
<!-- see if there's a linear model that can have exp as a fixed effect and random effect? -->
<!-- Add SEE for utility fits accross experiemetns -->
  <!-- AIC for this  -->
<!-- AIC/BIC For the model movement duration predictions -->

<!-- Utility that minimizes reaction time and movement time? -->
  <!-- add P/t_R where p is something to the numerator -->

<!-- 7/16 things to do -->
<!-- Check methods -->
<!-- Add reaction time into probability calculation? -->

<!-- 7/21 Done -->
<!-- use alpha from two experiment fit in the  -->
<!-- In the predicting movement duration -->
<!-- add net utility to this -->
<!-- Add fit for edot with body mass replaced by effective mass -->
  <!-- Do AIC for these fits -->

<!-- Done 7/20 -->
<!-- Refit utility models with the a_0 -->
<!-- Check the probability fits as they look to good, something about hte smoothing function is off -->
  <!-- Make it go to zero -->
<!-- Find average probability of succes for pref experiements using function -->
  <!-- Add to table -->
  <!-- Do for subject averages -->

<!-- Done 7/17 -->
<!-- Parameter fit standard errors in table -->
<!-- Simulate torque squared fro mmin jerk... -->
  <!-- Can do this with range of masses and speeds -->
<!-- Fit subject mass of arm in effort models to test -->

<!-- Done 7/15 -->
<!-- add sum of torque squared from minimjerk, its int the curent bio -->
<!-- Add figure as well for sum of torque rate -->
<!-- Get coefficients and equation for the probability stuff -->
<!-- Check conf intervals on parameter estiamtes -->
<!-- SSE for normalized stuff -->
<!-- Remove Mass1 -->
<!-- Add MP net into metabolic resuts -->
...
smalltdata <- read.csv('F:/Google Drive/Preferred Mass/FullData_SmallTargetnoFML_test.csv')
...
The probabilites and movement durations for the combined alpha are showin at the end, in section Utility fits accross experiements.
...
opt_probs = cbind(inv.logit(predict(smallt_p_glm,data.frame(movedur=util_dur_net_rx,eff_mass=masses))),
                  inv.logit(predict(smallt_p_glm,data.frame(movedur=util_dur_gross_rx,eff_mass=masses))))
...
comb_ut_probtab = cbind(round(inv.logit(predict(smallt_p_glm,data.frame(movedur=util_dur_pref_alpha_comb_gross[1:4],eff_mass=masses))),4),
                        round(inv.logit(predict(smallt_p_glm,data.frame(movedur=util_dur_pref_alpha_comb_gross[5:8],eff_mass=masses))),4),
                        c(1,1,1,1))
...
alph_tab = cbind(round(alpha_pref_comb_gross$minimum,3), round(alpha_pref_comb_gross$minimum,3), round(alpha_pilot$minimum,3))
colnames(alph_tab) = c('2a','2b','2c')
rownames(alph_tab) = '\alpha'

```

```{r comb_ut_durtab1, echo = FALSE, warning = FALSE, tab.cap = 'Experimental and predicted movement durations for the experiments.'}
kable(comb_ut_durtab, caption = 'Experimental and predicted movement durations for the experiments.')
```

```{r comb_ut_probtab1, echo = FALSE, warning = FALSE, tab.cap = 'Probabilites of success for the experiments.'}
kable(comb_ut_probtab, caption = 'Probabilites of success for the experiments.')
```

```{r alph_tab1, echo = FALSE, warning = FALSE, tab.cap = 'Alpha values fitted to experiment 2a/2b, and a seperate alpha for 2c.'}
