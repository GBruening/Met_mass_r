...
tab2 = cbind(c(NaN,aggregate(movedur ~ eff_mass,prefdata,mean)$movedur,NaN),
             c(NaN,met_net_dur,sum((met_net_dur-mvttimes_pref)^2)),
             c(NaN,met_gross_dur,sum((met_gross_dur-mvttimes_pref)^2)),
             c(round(alpha_utility_net_rx$minimum,4),util_dur_net_rx,sum((util_dur_net_rx-mvttimes_pref)^2)),
             c(round(alpha_utility_gross_rx$minimum,4),util_dur_gross_rx,sum((util_dur_gross_rx-mvttimes_pref)^2)),
             c(round(alpha_torque_2a,4),torque_dur_2a,sum((torque_dur_2a-mvttimes_pref)^2)),
             c(round(alpha_torque_2a_mj,4),torque_mj_dur_2a,sum((torque_mj_dur_2a-mvttimes_pref)^2)))
tab2 = round(tab2,4)
...
```{r alphavalues2b, echo = FALSE, warning = FALSE, message = FALSE, tab.cap = '$\alpha$ fits and SSE for the different experiements'}
tab = cbind(c('Utility 2b Gross',
              'Utility 2b Net'),
            round(c(alpha_smallt_gross$minimum,
                    alpha_smallt_net$minimum),3),
            c(formatC(sum((mvttimes_smallt-util_dur_smallt_gross)^2),format = 'e', digits = 3),
              formatC(sum((mvttimes_smallt-util_dur_smallt_net)^2),format = 'e', digits = 3)))
colnames(tab) = c('Experiments fit','$\alpha$','SSE')
kable(tab, caption = '$\alpha$ fits and SSE for the different experiements.')
```

These tables (@\ref{tab:utildurs2b}) show the predicted utility durations given one alpha fit to 2b 

The probability of success for utility in experiement 2b using the optimized alpha value are shown below (tab \@{ref:probs2atab}.

```{r probs2atab, echo = FALSE, warning = FALSE, message = FALSE}
  
opt_probs = cbind(inv.logit(predict(smallt_p_glm,data.frame(movedur=util_dur_smallt_gross,eff_mass=masses))),
                  inv.logit(predict(smallt_p_glm,data.frame(movedur=util_dur_smallt_net,eff_mass=masses))))

rownames(opt_probs) = masses
colnames(opt_probs) = c('Utility Gross','Utility Net')
...
colnames(twob_alpha_durs) = c('2b Experim','2b Predict Gross Util','2b Predict Net Util')
twob_alpha_durs <- round(twob_alpha_durs,3)
kable(twob_alpha_durs, caption = 'One alpha to 2b. Gross and net cost as effort term.')
...
```{r alphavalues2c, echo = FALSE, warning = FALSE, message = FALSE, tab.cap = '$\alpha$ fits and SSE for the different experiements'}
tab = cbind(c('Utility 2c Gross',
              'Utility 2c Net'),
            round(c(alpha_pilot_gross$minimum,
                    alpha_pilot_net$minimum),3),
            c(formatC(sum((mvttimes_pilot-util_dur_pilot_gross)^2),format = 'e', digits = 3),
              formatC(sum((mvttimes_pilot-util_dur_pilot_net)^2),format = 'e', digits = 3)))
colnames(tab) = c('Experiments fit','$\alpha$','SSE')
kable(tab, caption = '$\alpha$ fits and SSE for the different experiements.')
```

These tables (@\ref{tab:utildurs2c}) show the predicted utility durations given one alpha fit to 2c. 
Experiment 2c had probabilities equal to 1.
...
colnames(twoc_alpha_durs) = c('2c Experim','2c Predict Gross Util','2b Predict Net Util')
twoc_alpha_durs <- round(twoc_alpha_durs,3)
kable(twoc_alpha_durs, caption = 'One alpha to 2c. Gross and net cost as effort term.')
