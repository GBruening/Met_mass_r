...
add_norm_var <- function(data, variables){
  for (vari in variables){
    zero_vals = data[data$eff_mass == '2.47',]
    for (k in 1:length(data$subj)){
      data[k,paste(vari,'_norm',sep='')] = data[k,vari]/zero_vals[zero_vals$subj == data[k,'subj'],vari]
    }
  }
...
fun_optim_prx_ut2 <- function(alpha,masses,mvttimes,p_alpha,rxtimes,opt_or_dur){
  ...
  for (k in 1:length(mvttimes)){
    ...
    count = 1
    for (t in times){
      ut[k, count] = -a0*rt-a*t+b*(m^c)/(t^(d-1))#alpha*p
      count = count+1
    }
    dur[k] = times[which.max(ut[k,])]
  }
...
Utility 3:
$$J = \frac{\alpha P(R|m,T_m)}{T} -\left( a_0 T_r + a T_m + \frac{bm^c}{T_m^d} \right)$$

```{r, echo = FALSE, warning = FALSE}
#2a =========================================================
ut_2a_tab = cbind(c('2.47','3.80','4.70','6.10'),
              mvttimes_pref,
              ut1_dur_2a,
              ut2_dur_2a,
              ut3_dur_2a,
              ...
              ut2_dur_2a2b[1:4],
              ut3_dur_2a2b[1:4])
colnames(ut_2a_tab)<-c('eff_mass',colnames(ut_2a_tab)[2:5],'utcomb1_dur_2a','utcomb2_dur_2a','utcomb3_dur_2a')
kable(ut_2a_tab %>% `colnames<-`(c('eff_mass','MOvement Duration (s)',
                                   'Utility 1 Duration','Utility 2 Duration','Utility 3 Duration',
                                   'Utility Comb 1 Duration','Utility Comb 2 Duration','Utility Comb 3 Duration')),
      caption = '2a - Movement duration by utility model for experiment 2a.')

masses = c('2.47','3.80','4.70','6.10')
ut_2a_df = as.data.frame(rbind(cbind(masses,rep('Exp',4),mvttimes_pref),
                 ...
                 cbind(masses,rep('Utcomb3',4),ut3_dur_2a2b[1:4])) %>% `colnames<-`(c('eff_mass','data','movedur')))
...
ut_2a_plot <- ggplot()+
  ...
  geom_point(data=ut_2a_df,
             aes(x=eff_mass,
                 ...
                 color = factor(data)),
            size = 2)+
  labs(x = 'Effective Mass (kg)', y = 'Predicted Movement Duration (s)', color = 'Data')
  
#2b =========================================================
ut_2b_tab = cbind(c('2.47','3.80','4.70','6.10'),
              mvttimes_pref,
              ut1_dur_2b,
              ut2_dur_2b,
              ut3_dur_2b,
              ...
              ut2_dur_2a2b[5:8],
              ut3_dur_2a2b[5:8])
colnames(ut_2b_tab)<-c('eff_mass',colnames(ut_2b_tab)[2:5],'utcomb1_dur_2b','utcomb2_dur_2b','utcomb3_dur_2b')
kable(ut_2b_tab %>% `colnames<-`(c('eff_mass','MOvement Duration (s)',
                                   'Utility 1 Duration','Utility 2 Duration','Utility 3 Duration',
                                   'Utility Comb 1 Duration','Utility Comb 2 Duration','Utility Comb 3 Duration')),
      caption = '2a - Movement duration by utility model for experiment 2a.')
...
ut_2b_df = as.data.frame(rbind(cbind(masses,rep('Exp',4),mvttimes_smallt),
                 ...
                 cbind(masses,rep('Utcomb3',4),ut3_dur_2a2b[5:8])) %>% `colnames<-`(c('eff_mass','data','movedur')))
...
#2c =========================================================
ut_2c_tab = cbind(c('2.47','3.80','4.70','6.10'),
              mvttimes_pilot,
              ut1_dur_2c,
              ut2_dur_2c,
              ut3_dur_2c)
colnames(ut_2c_tab)<-c('eff_mass',colnames(ut_2c_tab)[2:5])
kable(ut_2c %>% `colnames<-`(c('MOvement Duration (s)','Utility 1 Duration','Utility 2 Duration','Utility 3 Duration')),
      caption = '2c - Movement duration by utility model for experiment 2c.')
...
ut_2c_df = as.data.frame(rbind(cbind(masses,rep('Exp',4),mvttimes_pilot),
                 ...
                 cbind(masses,rep('Ut3',4),ut3_dur_2c)) %>% `colnames<-`(c('eff_mass','data','movedur')))
...
ut_2c_plot <- ggplot()+
  ...
  geom_point(data=ut_2c_df,
             aes(x=eff_mass,
                 ...
                 color = factor(data)),
            size = 2)+
  scale_color_manual(values = gg_color_hue(7))+
  ylim(c(.6,.8))+
  labs(x = 'Effective Mass (kg)', y = 'Predicted Movement Duration (s)', color = 'Data')#, title = 'ut2/ut3 not shown cause it doesn\'t fit.')
...
# setwd(pref_plots_dir)
# if (save_plots){ggsave(filename='alt_ut_mod.pdf',plot=alt_ut_mod,height=8,width=6,useDingbats=FALSE)}
```
