---
title: "Mass Tests No Reaction Time in Utility"
...
avg_plot2 <- function(variable,data,ylab,shape_labs,...){
  ...
  if (!missing(shape_labs)){
    g <- g+scale_shape_manual(values = c(16,17,15),labels = shape_labs)+labs(shape = 'Algorithm')
  }
  return(g)
}
...
reaction_tanv_algo[['pref']] <- avg_plot2(c('reaction_tanv','reaction_erik','reaction_vthresh'),prefdata,ylab='Reaction Time (s)',shape_labs = c('Mine','Erik','V Thresh'))

reaction_tanv_algo[['smallt']] <- avg_plot2(c('reaction_tanv','reaction_erik','reaction_vthresh'),smalltdata,ylab='Reaction Time (s)',shape_labs = c('Mine','Erik','V Thresh'))

reaction_tanv_algo[['pilot']] <- avg_plot2(c('reaction_tanv','reaction_erik','reaction_vthresh'),pilotdata,ylab='Reaction Time (s)',shape_labs = c('Mine','Erik','V Thresh'))

```

```{r reactiontanvalgoplot1, echo = FALSE, warning = FALSE, fig.width=12, fig.height=5, fig.cap = 'Reaction Time Algorithm method.'}

plot_grid(reaction_tanv_algo[['pref']]+theme(legend.position = 'none')+scale_y_continuous(breaks = seq(.17,.29,.02), limits = c(.17,.29)),
          reaction_tanv_algo[['smallt']]+theme(legend.position = 'none')+scale_y_continuous(breaks = seq(.17,.29,.02), limits = c(.17,.29)),
          reaction_tanv_algo[['pilot']]+theme(legend.position = 'none')+scale_y_continuous(breaks = seq(.17,.29,.02), limits = c(.17,.29)),
...
reaction_tanvel_algo[['pref']] <- avg_plot2(c('reaction_tanvel','onsetErik_vel','onsetvthresh_vel'),
                                            prefdata,ylab='Reaction Velocity (m/s)',
                                            shape_labs = c('Mine','Erik','V Thresh'))

reaction_tanvel_algo[['smallt']] <- avg_plot2(c('reaction_tanvel','onsetErik_vel','onsetvthresh_vel'),
                                              smalltdata,ylab='Reaction Velocity (m/s)',
                                              shape_labs = c('Mine','Erik','V Thresh'))

reaction_tanvel_algo[['pilot']] <- avg_plot2(c('reaction_tanvel','onsetErik_vel','onsetvthresh_vel'),
...
plot_grid(reaction_tanvel_algo[['pref']]+theme(legend.position = 'none')+scale_y_continuous(breaks = seq(0,.016,.004), limits = c(-.001,.016)),
          reaction_tanvel_algo[['smallt']]+theme(legend.position = 'none')+scale_y_continuous(breaks = seq(0,.016,.004), limits = c(-.001,.016)),
          reaction_tanvel_algo[['pilot']]+theme(legend.position = 'none')+scale_y_continuous(breaks = seq(0,.016,.004), limits = c(-.001,.016)),
...
fun_optim_prx <- function(alpha,masses,mvttimes,p_alpha,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    for (t in times){
      ut[count] = (alpha*p-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
      count = count+1
    }
...
fun_optim_prx <- function(alpha,masses,mvttimes,p_alpha,rxtimes,opt_or_dur){
  ...
  for (k in 1:length(mvttimes)){
    ...
    count = 1
    for (t in times){
      ut[k, count] = (alpha*p-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
      count = count+1
    }
...
fun_optim_prx_logit <- function(alpha,masses,mvttimes,p_alpha,rxtimes,opt_or_dur){
  ...
  for (k in 1:length(mvttimes)){
    ...
    count = 1
    for (t in times){
      ut[count] = (alpha*probs[count]-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
    # print(max(ut))
...
fun_optim_torque_prx <- function(alpha,masses,mvttimes,p_alpha,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    for (t in times){
    t_count = t_count+1
      ut[count] = (alpha*probs[t_count]-(0*t+b_torque*(m^c_torque)/(t^(d_torque-1))))/(1+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
fun_optim_torque_prx2 <- function(alpha,masses,mvttimes,probs,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    for (t in times){
    t_count = t_count+1
      ut[count] = (alpha*p-(0*t+b_torque*(m^c_torque)/(t^(d_torque-1))))/(1+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
fun_optim_torque_prx_mj <- function(alpha,masses,mvttimes,p_alpha,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    for (t in times){
    t_count = t_count+1
      ut[count] = (alpha*probs[t_count]-coef(MPGross_model)[1]*rt-(0*t+b_torque_mj*(m^c_torque_mj)/(t^(d_torque_mj-1))))/(1+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
fun_optim_torque_prx_mj2 <- function(alpha,masses,mvttimes,probs,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    for (t in times){
    t_count = t_count+1
      ut[count] = (alpha*p-(0*t+b_torque_mj*(m^c_torque_mj)/(t^(d_torque_mj-1))))/(1+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
# count = 1
# for (t in times){
#   ut100[count] = (alpha100*probs[count]-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
#   count = count+1
# }
...
# count = 1
# for (t in times){
#   ut1000[count] = (alpha1000*probs[count]-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
#   count = count+1
# }
...
# count = 1
# for (t in times){
#   ut5000[count] = (alpha5000*probs[count]-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
#   count = count+1
# }
...
#   count = 1
#   for (t in times){
#     ut[count,alph_c] = (alpha100*probs[count]-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
#     count = count+1
#   }
...
fun_optim_prx_logit_comb <- function(alpha,masses,mvttimes,glm1,glm2,rxtimes,opt_or_dur){
  ...
  for (k in 1:length(mvttimes)){
    ...
    for (t in times){
    t_count = t_count+1
      ut[count] = (alpha*probs[t_count]-(a*t+b*(m^c)/(t^(d-1))))/(1+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
for (model1 in models_SSE){
  ...
  model_peakvel_temp1_norm = filter(model_vels,model==model1)$norm_vels
  # Change here for 2a/b/c
  model_peakvel_temp2_norm = aggregate(peakvel_target~eff_mass,smalltdata,mean)$peakvel_target/aggregate(peakvel_target~eff_mass,smalltdata,mean)$peakvel_target[1]
  SSE = sum((model_peakvel_temp1_norm-model_peakvel_temp2_norm)^2)
  Model_SSE_peakvel_norm = rbind(Model_SSE_peakvel_norm,SSE)
  # Model_SSE_colnames = rbind(Model_SSE_colnames,model1)
