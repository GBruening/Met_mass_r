...
all_exp_avg_plot <- function(variable,data1,data2,data3,ylab,...){
  ...
  g<-ggplot()+
    ...
    geom_hline(aes(yintercept = filter(c,exp=='Arc')[1,2]),
               linetype = 'dashed',
               size=.5)+
    geom_hline(aes(yintercept = filter(c,exp=='No Target')[1,2]),
               linetype = 'dashed',
               size=.5)+
    geom_errorbar(data=data.frame(c),
                  aes(x    = eff_mass,
...
```{r, echo = FALSE}
  ...
  colnames(move_dur) <- c('Effective Mass (kg)','Circle Target','Arc Target','No Target')
...
```{r movedur1, echo = FALSE, warning = FALSE, fig.width=12, fig.height=8, fig.cap = 'Movement duration broken up by experiment.'}
  plot_grid(movedur[['pref']],
            ...
            nrow = 2,
            align = 'vh',
            labels = c('Preferred','Smallt','No Target','','',''))
```
...
```{r, echo = FALSE}
  ...
  colnames(peak_vel) <- c('Effective Mass (kg)','Circle Target','Arc Target','No Target')
...
```{r, echo = FALSE}
  ...
  colnames(react_time) <- c('Effective Mass (kg)','Circle Target','Arc Target','No Target')
...
colnames(miss_dist_time) <- c('Effective Mass (kg)','Circle Target','Arc Target','No Target')
...
colnames(missangle_time) <- c('Effective Mass (kg)','Circle Target','Arc Target','No Target')
...
colnames(miss_rad_time) <- c('Effective Mass (kg)','Circle Target','Arc Target','No Target')
...
pref_results = plot_grid(plot_grid(NULL,
                                       ...
                                       exp_plots[['pilot']],
                                       nrow = 6,
                                       labels = c('Circle Target','','Arc Target','','No Target'),
                                       # labels = 'AUTO',
...
if (save_plots){
  setwd(met_plots_dir)
  ggsave('MPGross_by_movedur_met.pdf',plot=MPGross,height = 4, width = 4, useDingbats = FALSE, path = met_plots_dir)
}
...
b_mpnet=coef(MPGross_model)[2]
c_mpnet=coef(MPGross_model)[3]
d_mpnet=coef(MPGross_model)[4]
...
pdata = cbind(aggregate(movedur ~ speed + effmass,mpdata_factor,mean),
              aggregate(movedur ~ speed + effmass,mpdata_factor,sd)$movedur/sqrt(8),
              aggregate(metpowernet ~ speed + effmass,mpdata_factor,mean)$metpowernet,
              aggregate(metpowernet ~ speed + effmass,mpdata_factor,sd)$metpowernet/sqrt(8))
colnames(pdata) = c('speed','effmass','movedur','movedurse','metpowernet','metpowernetse')

MPNet <- ggplot(mpdata_factor,
...
setwd(met_plots_dir)
if (save_plots){ggsave('MPNet_by_movedur.pdf',plot = MPNet,height = 4, width = 4, useDingbats = FALSE)}
...
if (save_plots){
  setwd(met_plots_dir)
  ggsave('MCostGross_by_movedur_met.pdf',plot=MCGross,height = 4, width = 4, useDingbats = FALSE, path = met_plots_dir)
}
...
setwd(met_plots_dir)
if (save_plots){
  ggsave('MCostNet_by_movedur.pdf',plot=MCNet,height = 4, width = 4, useDingbats = FALSE, path = met_plots_dir)
}
...
Torque squared plots.

```{r torqueplot1, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = 'Sum of torque squared fits from the data'}
torque_plot
```

<!-- # ```{r torquemjplot1, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = 'Sum of torque squared fits from minimum jerk'} -->
<!-- # torque_mj_plot -->
<!-- # ``` -->


This table shows the confident intervals on the torque model. The probability of $a_1$ being greater than 0 is `r dnorm(0, coef(modelsum_torque2)[1,1] , coef(modelsum_torque2)[1,2])`.

```{r, echo = FALSE, warning = FALSE}
kable(confint(torque2_model), caption = 'Confidence intervals for torque squared model.')
```

<!-- This table shows the confident intervals on the torque model from minimum jerk. The probability of $a_1$ being greater than 0 is `r dnorm(0, coef(modelsum_torque_mj)[1,1] , coef(modelsum_torque_mj)[1,2])`. -->
<!-- ```{r, echo = FALSE, warning = FALSE} -->
<!-- kable(confint(torque2_mj_model), caption = 'Confidence intervals for torque squared minimum jerk model.') -->
<!-- ``` -->


## Table for all effort model parameters
This table shows all the parameters that were fitted in the effort models.
```{r , echo = FALSE, warning = FALSE, message = FALSE}
...
## Section for probability of sucess in just 2a and 2b without 1

## Probability of success using experiment 1
This plot shows the prbability of success as a function of movement duration and mass.
```{r, echo = FALSE}

metdata$p_success = abs(metdata$miss_dist)<.014
met_p_glm  = glm(p_success ~ movedur+eff_mass,
                 data=metdata,
                 family=binomial(link="logit"))
p_success_met = inv.logit(predict(met_p_glm))
metdata$p_success_met = p_success_met

metdata$p_success_smallt = abs(metdata$maxex)<.11 & abs(metdata$missangle)<3
smallt_p_glm  = glm(p_success_smallt ~ movedur+eff_mass,
                    data=metdata,
                    family=binomial(link="logit"))
p_success_smallt = inv.logit(predict(smallt_p_glm))
metdata$p_success_smallt = p_success_smallt
...
variable = 'p_success_met'
variable2 = 'p_success_smallt'
string = paste('
...
# fun.1 <- function(t) inv.logit(predict(met_p_glm,newdata = data.frame('eff_mass' = 2.47,'movedur'=t)))
# fun.2 <- function(t) inv.logit(predict(met_p_glm,newdata = data.frame('eff_mass' = 4.73,'movedur'=t)))
# fun.3 <- function(t) inv.logit(predict(met_p_glm,newdata = data.frame('eff_mass' = 5.99,'movedur'=t)))
# fun.4 <- function(t) inv.logit(predict(met_p_glm,newdata = data.frame('eff_mass' = 11.50,'movedur'=t)))
...
fun.5 <- function(t) 1/(1+exp(-coef(smallt_p_glm)[1]-coef(smallt_p_glm)[3]*2.47-coef(smallt_p_glm)[2]*t))
fun.6 <- function(t) 1/(1+exp(-coef(smallt_p_glm)[1]-coef(smallt_p_glm)[3]*4.73-coef(smallt_p_glm)[2]*t))
fun.7 <- function(t) 1/(1+exp(-coef(smallt_p_glm)[1]-coef(smallt_p_glm)[3]*6.99-coef(smallt_p_glm)[2]*t))
fun.8 <- function(t) 1/(1+exp(-coef(smallt_p_glm)[1]-coef(smallt_p_glm)[3]*11.50-coef(smallt_p_glm)[2]*t))
...
p_alpha_plot_cont <- ggplot(data=metdata,
  ...
  scale_shape_manual(values = c(21,22),
                     labels = c('Circle Target','Arc Target'))+
  guides(color = guide_legend(title="Effective\nMass (kg)"),
...
# #
# temp = data.frame(cbind(min(aggregate(movedur~eff_mass,prefdata,mean)$movedur),max(aggregate(movedur~eff_mass,prefdata,mean)$movedur)))
# colnames(temp) = c('min','max')
# temp2 = data.frame(cbind(min(aggregate(movedur~eff_mass,smalltdata,mean)$movedur),max(aggregate(movedur~eff_mass,smalltdata,mean)$movedur)))
# colnames(temp2) = c('min','max')
#   geom_errorbarh(data = temp,
#                  inherit.aes = FALSE,
#                  aes(xmin = min,
#                      xmax = max,
#                      y = .2,
#                      size = 3))+
#   geom_errorbarh(data = temp2,
#                  inherit.aes = FALSE,
#                  aes(xmin = min,
#                      xmax = max,
#                      y = .3,
#                      size = 3))+
#   annotate('text', x = temp$min+.3, y = .25, label = 'Circle Duration Range')+
#   annotate('text', x = temp2$min+.3, y = .35, label = 'Arc Duraiton Range')+
#     
# geom_linerange(data = data.frame('movedur' = aggregate(movedur~eff_mass,prefdata,mean)$movedur,
#                                  'prob' = inv.logit(predict(met_p_glm,newdata = aggregate(movedur~eff_mass,prefdata,mean)))),
#                aes(x = movedur,
#                    ymax = prob,
#                    ymin = 0))+
# geom_linerange(data = data.frame('movedur' = aggregate(movedur~eff_mass,smalltdata,mean)$movedur,
#                                  'prob' = inv.logit(predict(smallt_p_glm,newdata = aggregate(movedur~eff_mass,smalltdata,mean)))),
#                aes(x = movedur,
#                    ymax = prob,
#                    ymin = 0),
#                linetype = 'dashed')+
...
# line_types = c('line_pref' = 'solid','line_smallt' = 'dashed')
# ggplot()+
#   geom_smooth(data = filter(metdata,odd_trial==1),
#               aes(x=movedur,
#                   y=p_success_met,
#                   color=factor(condition)),
#               linetype = 'solid',
...
# =========================================================================
# sim_success = as.data.frame(cbind(c(rep(2.46,1000),rep(4.73,1000),rep(6.99,1000),rep(11.50,1000)),
#                             c(seq(.4,1.6,length.out=1000),seq(.4,1.6,length.out=1000),seq(.4,1.6,length.out=1000),seq(.4,1.6,length.out=1000))))
# colnames(sim_success) = c('eff_mass','movedur')
# sim_success$p_success_met = inv.logit(predict(met_p_glm,newdata = sim_success))
# sim_success$p_success_smallt = inv.logit(predict(smallt_p_glm,newdata = sim_success))
# 
# line_types = c('line_pref' = 'solid','line_smallt' = 'dashed')
# p_alpha_plot_cont <- ggplot()+
#   geom_smooth(data = sim_success,
#               aes(x=movedur,
#                   y=p_success_met,
#                   color=factor(eff_mass)),
#               linetype = 'solid',
...
The probability function for the circle target is:
$$ ln\left(\frac{P_i}{1-P_i}\right) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 $$
Which leads to
$$ P(Success|m,T) = \frac{1}{1+e^{-(\beta_0) - (\beta_1)m - (\beta_2)T}} $$


<!-- which leads to the following equation where $m$ is mass ant $T$ is movement duration. -->
<!-- $$ P(Success|m,T) = \frac{1}{1+e^{-(-1.446) - (-0.096)m - (5.876)T)}} $$ -->

<!-- For the arc target: -->
<!--   $$ P(Success|m,T) = \frac{1}{1+e^{-( `r paste(round(coef(smallt_p_glm)[1],4))` ) - ( `r paste(round(coef(smallt_p_glm)[3],4))` )m - ( `r paste(round(coef(smallt_p_glm)[2],4))` )T)}} $$ -->
...
tab = cbind(c(paste(round(coef(met_p_glm)[1],4),'±',round(coef(summary(met_p_glm))[1,2],3),sep=' '),
            ...
            c(paste(round(coef(smallt_p_glm)[1],4),'±',round(coef(summary(smallt_p_glm))[1,2],3),sep=' '),
              paste(round(coef(smallt_p_glm)[3],4),'±',round(coef(summary(smallt_p_glm))[3,2],3),sep=' '),
              paste(round(coef(smallt_p_glm)[2],4),'±',round(coef(summary(smallt_p_glm))[2,2],3),sep=' ')))
rownames(tab) = c('$\\beta_0$','$\\beta_1$','$\\beta_2$')
colnames(tab) = c('Circle Target','Arc Target')
kable(tab, caption = 'Beta coefficient for the inverse logit function to predict probability of success.')
```

This plot only includes movement durations that are seen in the metabolic experiment. The black vertical bars show the range of durations for the circle target preferred exerpiement. The red vertical bars show the range of durations for the smallt target.

```{r palphaplotcont1, echo = FALSE, warning = FALSE, fig.width=9, fig.height=6, fig.cap = 'Probability of success given movement duration and mass. The black vertical bars show the range of durations for the circle target preferred exerpiement. The red vertical bars show the range of durations for the smallt target.'}
  p_alpha_plot_cont
```
...
```{r palphaplotcont2, echo = FALSE, warning = FALSE, fig.width=9, fig.height=6, fig.cap = 'Probability of success given movement duration and mass. The black vertical bars show the range of durations for the circle target preferred exerpiement. The red vertical bars show the range of durations for the smallt target.'}
  p_alpha_plot_cont2
```

This next table shows the movement durations for the circle target and small target along with the probability of success from the logistic regression. The last row is the mean probability.
...
movedur_probs = cbind(c(aggregate(movedur~eff_mass,prefdata,mean)$movedur,NaN),
                      c(pref_probs,mean(pref_probs)),
                      c(aggregate(movedur~eff_mass,smalltdata,mean)$movedur,NaN),
                      c(smallt_probs,mean(smallt_probs)))
movedur_probs = round(movedur_probs,4)
rownames(movedur_probs) = c('2.47','3.80','4.70','6.10','Mean')
colnames(movedur_probs) = c('Circle Movedur','Circle Probability','Smallt Movedur','Smallt Probability')
```

```{r movement_duration_probs1, echo = FALSE, warning = FALSE, tab.cap = 'Probability of success for the preferred experiment.'}
kable(movedur_probs)
```
...
prob_dur_opt <- function(p_thresh,mvttimes,exp_glm){
  ...
  for (mass in c(2.47,3.8,4.7,6.1)){
    for (dur in seq(.3,1.5,.001)){
      if (inv.logit(predict(exp_glm,data.frame(movedur=dur,eff_mass=mass)))>p_thresh){
...
pref_tab = cbind(c('2.47','3.8','4.7','6.1'),prob_dur_pref,round(mvttimes,3))
...
The probability of reward that minimizes the error of predicted movement durations for the circle target experiment is `r round(prob_dur_p_thresh$minimum,4)`. The table below shows the predicted movement durations and the expected movement durations. The SSE of this prediction is `r accuracy_SSE_pref`.

```{r prefprobtab1, echo = FALSE}
  kable(pref_tab, caption = paste('Predicted movement durations for the circle target given probability of: ',round(prob_dur_p_thresh$minimum,4),sep=''))
```

The probability of reward that minimizes the error of predicted movement durations for the circle target experiment is `r round(prob_dur_p_thresh_smallt$minimum,4)`. The table below shows hte predicted movement durations and the expected movement durations. The SSE of this prediction is `r accuracy_SSE_smallt`.

```{r smalltprobtab1, echo = FALSE}
  kable(smallt_tab, caption = paste('Predicted movement durations for the ARC target given probability of: ',round(prob_dur_p_thresh_smallt$minimum,4),sep=''))
```

# Utility Modeling

This plot below shows individual subjects and their utility fits.
...
p_alpha_smallt = inv.logit(predict(smallt_p_glm,data.frame(movedur=mvttimes_smallt,eff_mass=masses)))
...
util_dur_comb_alpha_gross = as.numeric(fun_optim_prx(39.17942,
                                       masses = masses,
                                       mvttimes = mvttimes,
                                       p_alpha  = p_alpha_pref,
                                       opt_or_dur = 'duration',
...
util_dur_comb_alpha_net = as.numeric(fun_optim_prx(39.17942,
                                       masses = masses,
                                       mvttimes = mvttimes,
                                       p_alpha  = p_alpha_pref,
                                       opt_or_dur = 'duration',
...
#   ut = rep(0,length(times))
#   for (t in times){
#     ut[count] = (alpha*p-a*rt-(a*t+b*(m^c)/(t^(d-1))))/(rt+t)
#     count = count+1
#   }
#   dur[k] = times[which.max(ut)]
# }
...
fun_optim_torque_prx <- function(alpha,masses,mvttimes,p_alpha,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    ut = rep(0,length(times))
    for (t in times){
      ut[count] = (alpha*p-coef(MPGross_model)[1]*rt-(0*t+b_torque*(m^c_torque)/(t^(d_torque-1))))/(rt+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
fun_optim_torque_prx_mj <- function(alpha,masses,mvttimes,p_alpha,rxtimes){
  ...
  for (k in 1:length(mvttimes)){
    ...
    ut = rep(0,length(times))
    for (t in times){
      ut[count] = (alpha*p-coef(MPGross_model)[1]*rt-(0*t+b_torque_mj*(m^c_torque_mj)/(t^(d_torque_mj-1))))/(rt+t)
      count = count+1
    }
    dur[k] = times[which.max(ut)]
  }
...
The tables below show the movement durations (@\ref{rab:util_movedurs_tab}), reaction time (@\ref{rab:util_reacttimes_tab}), and probability of success (@\ref{rab:util_probs_tab}).
...
probs = cbind(round(p_alpha_pref,4),
...
alpha_pref=optimize(fun_optim_prx,
...
alpha_smallt=optimize(fun_optim_prx,
                      masses   = masses,
                      mvttimes = mvttimes_smallt,
                      p_alpha  = p_alpha_smallt,
...
util_dur_smallt = as.numeric(fun_optim_prx(alpha_pref$minimum, #changing between alpha_pref and alpha_smallt
                                     masses   = masses,
                                     mvttimes = mvttimes_smallt,
                                     p_alpha  = p_alpha_smallt,
                                     rxtimes  = rxtimes_smallt,
                                     opt_or_dur = 'duration'))
util_dur_pref = as.numeric(fun_optim_prx(alpha_pref$minimum,
                                   masses   = masses,
...
The next table shows the alpha values for many of the experiements.

```{r alpha_values, echo = FALSE, warning = FALSE, message = FALSE, tab.cap = '$\alpha$ fits for the different experiements'}
tab = cbind(c('Utility Gross 2a','Utility Net 2a','Utility Gross 2a and 2b','Utility 2b Gross','Utility 2c Gross'),
            round(c(alpha_utility_gross_rx$minimum,
                    alpha_utility_net_rx$minimum,
                    alpha_pref$minimum,
...
model_durs = data.frame('eff_mass' = cbind(rep(masses,13)),
                        ...
                        'movedur' = c(aggregate(movedur~eff_mass,prefdata,mean)$movedur,
                                     ...
                                     util_dur_gross_rx,
                                     util_dur_net_rx,
                                     util_dur_comb_alpha_gross,
                                     util_dur_comb_alpha_net,
                                     m1_dur,
                                     m2_dur,
                                     torque_dur,
                                     prob_dur_pref),
...
for (c in 1:4){
  ...
  util_peakvel_comb_alpha_gross[c] = min_jerk_vel(util_dur_comb_alpha_gross[c])
  util_peakvel_comb_alpha_net[c] = min_jerk_vel(util_dur_comb_alpha_net[c])
...
This table shows the SSE for the circle target movement duration and peak velocity predictions.
...
util_2b_from_2a = optimize(fun_optim_prx,
                                  masses = masses,
                                  mvttimes = mvttimes,
                                  p_alpha  = p_alpha_pref,
                                  rxtimes = rxtimes,
...
util_dur_2b_from_2a = as.numeric(fun_optim_prx(util_2b_from_2a$minimum,
                                       masses = masses,
                                       mvttimes = mvttimes_smallt,
                                       p_alpha  = p_alpha_smallt,
                                       opt_or_dur = 'duration',
...
The small target probabilities used in the utility model are `r round(p_alpha_smallt,4)`. For the no target, there is no probability so this is just set to 1. 
```{r, echo = FALSE, warning = FALSE}
...
control_times = rbind(cbind(rep('smallt',4),rep('data',4),c(2.5,3.8,4.7,6.1),mvttimes_smallt,rep(3,4),mvttimes_smallt_se),
                      ...
                      cbind(rep('smallt',4),rep('model',4),c(2.5,3.8,4.7,6.1),util_dur_smallt,rep(3,4),rep(0,4)),
                      cbind(rep('pref',4),rep('model',4),c(2.5,3.8,4.7,6.1),util_dur_pref,rep(1,4),rep(0,4)),
...
utilfits_by_exp <- ggplot()+
  ...
  labs(x='Effective mass (kg)',
       y='Movement Duration (s)',
       title = 'Utility Fits By Experiment')+
  scale_color_manual(labels = c('Circle\n(Normal)','None\n(Large)','Arc\n(Small)'),
                     values = gg_color_hue(3))+
