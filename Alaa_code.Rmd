...
dur[k] = times[which.max(ut)]
plot(ut[500:length(ut)])


alpha2    = 10000000
ut2 = rep(0,length(times))
count = 1
for (t in times){
  ut2[count] = (alpha2*probs[count]-a0*rt-(a*t+b*(m^c)/(t^(d-1))))/(rt+t)
  count = count+1
}
dur2[k] = times[which.max(ut)]

test_data = data.frame(cbind(c(rep(alpha,length(times)),rep(alpha2,length(times))),c(times,times),c(ut/max(ut),ut2/max(ut2))))
colnames(test_data) = c('alpha','movedur','utility')
ggplot()+geom_line(data=filter(test_data,utility>0),aes(x=movedur,y=utility,color=factor(alpha)))
